...
export default function WaveSetup() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto fade-in">
          ...
          {reminders.map((reminder, idx) => (
            <div key={reminder.id} className="border border-gray-100 rounded-xl p-3 bg-gray-50">
              <div className="flex items-center gap-2 mb-2">
                ...
                <Button variant="ghost" size="sm" onClick={() => removeReminder(reminder.id)} className="ml-auto text-gray-300 hover:text-red-400 !p-0.5"><Trash2 size={13} /></Button>
              </div>
              <input
                type="datetime-local"
                value={reminder.datetime}
                onChange={e => updateReminder(reminder.id, e.target.value)}
                className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none transition-colors bg-white ${reminder.error ? 'border-red-300' : 'border-gray-200 focus:border-purple-400'}`}
              />
              {reminder.error && (
      ...
      {/* Section 5 — Post-submission & Survey-closed Emails (P1-F-100, P1-F-101) */}
      <SectionCard number="5" title="Expert Notification Emails" icon={Bell}>
        <p className="text-xs text-gray-500 mb-4 leading-relaxed">
          These emails are sent automatically — the post-submission email fires immediately after an expert submits,
          and the survey-closed email fires when the survey closes and the report is ready.
          Templates pre-fill from your personal defaults. Expand to customise per survey.
        </p>

        {/* Post-submission email */}
        <div className="border border-gray-100 rounded-xl mb-3 overflow-hidden">
          <button
            onClick={() => setPostSubExpanded(v => !v)}
        ...
        <div className="border border-gray-100 rounded-xl overflow-hidden">
          <button
            onClick={() => setSurveyClosedExpanded(v => !v)}
